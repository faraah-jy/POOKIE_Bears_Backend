...
const sendNotifPresent = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      msg: "Notification has been stored successfully!",
      data: newNotifData,
    });
  } catch (err) {
    console.log(err);
...
const sendNotifAbsent = async (req, res) => {
  try {
    const msg = req.body.msg;
    if (!msg) {
      return res.status(400).json({ msg: "Missing message" });
    ...
    const users = await User.find({ persentChild: false });
    if (users.length === 0) {
      return res.status(404).json({ msg: "No users with absent Child found" });
    }
...
const getNotif = async (req, res) => {
  try {
    ...
    if(user.registerAuth){


    if (!notifications || notifications.length === 0) {
      return res
        .status(404)
        .json({ msg: "No notifications found for the user." });
    }
